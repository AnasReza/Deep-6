...
class Rateus extends StatefulWidget {
  ...
  Rateus(int chapterIndex, String language){
    this.chapterIndex = chapterIndex;
    this.language = language;
  }

  @override
  RateusState createState() => new RateusState();
}

class RateusState extends State<Rateus> with TickerProviderStateMixin {
  ...
  Future readJson(int chapter) async {
    try {
      var urlString = "assets/json/$lang/rateus.json";
      ...
      List<dynamic> rbArray = sceneArray.last['rightButton'];

      setState(() {
        mainText = textArray[0]['text'];
        leftBtnText = lbArray[0]['lbText'];
        rightBtnText = rbArray[0]['rbText'];
      });
  ...
  Widget build(BuildContext context) {
    ...
    var leftButton = new MaterialButton(
      key: null,
      onPressed: () {
        cPlay.playClick();
        if (isEnabled) {
            LaunchReview.launch(androidAppId: "de.atropos.deep6.pay", iOSAppId: "1439780808");
    ...
    var rightButton = new MaterialButton(
      key: null,
      onPressed: () {
        cPlay.playClick();
        if (isEnabled) {
